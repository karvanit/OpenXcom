 ...
 */
void operator>>(const YAML::Node &nd, WeightedOptions &wo)
{
 ...
 * @param wo The option list to send.
 * @return A reference to the emitter.
 */
YAML::Emitter &operator<<(YAML::Emitter &out, const WeightedOptions &wo)
{
	out << YAML::BeginMap;
	for (std::map<std::string, unsigned>::const_iterator ii = wo._choices.begin(); ii != wo._choices.end(); ++ii)
	{
